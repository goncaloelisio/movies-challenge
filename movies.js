...
function topWatchlistedMoviesAmongFriends(userId) {
  ...
  sortedByWatchCount = moviesWatchCounts.sort((a, b) => {
    if (a[1] > b[1]) return -1;
    // if they are equal, have nested if to determine order via alphabetical order (javascript can compare strings and return true or false based on alphabetical)
    if (a[1] === b[1]) {
      if (a[0] < b[0]) return -1;
      if (a[0] === b[0]) return 0;
      if (a[0] > b[0]) return 1;
  ...
  topFour = sortedByWatchCount.slice(0, 4).map((input) => {
    return input[0];
  });
